...
import { isAuthenticated } from "~/utils/auth/authHelper";
import { apiReadUserById } from "~/utils/apiUser";

const route = useRoute()
const user = useState('user', () => null)
const redirect = useState('redirect', () => false)


const checkForUserFollow = (user) => {
}
...
onMounted(async () => {
  ...
  if (jwt) {
    const { data } = await apiReadUserById(route.params.userID, jwt.token)
